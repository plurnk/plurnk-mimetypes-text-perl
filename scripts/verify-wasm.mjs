#!/usr/bin/env node
// Verifies the committed perl.wasm is byte-identical to what the pinned
...
const pin = (await readFile(path.join(repoRoot, ".perl-grammar-pin"), "utf-8")).trim();
const committedBytes = await readFile(path.join(repoRoot, "perl.wasm"));
const committedHash = createHash("sha256").update(committedBytes).digest("hex");
console.log(`committed perl.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "perl-wasm-verify-"));
...
const rebuiltBytes = await readFile(path.join(work, "src", "tree-sitter-perl.wasm"));
const rebuiltHash = createHash("sha256").update(rebuiltBytes).digest("hex");
console.log(`rebuilt perl.wasm sha256: ${rebuiltHash}`);

if (committedHash !== rebuiltHash) {
    console.error("FAIL: committed perl.wasm does not match rebuild from pinned source");
    process.exit(1);
}
